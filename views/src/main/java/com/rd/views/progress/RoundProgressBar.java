...
public class RoundProgressBar extends View {
    ...
    /** 出售 */
    private String  progressBarSaleTime;
    /** 中间进度百分比的字符串的颜色 */
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        textSize = mTypedArray.getDimensionPixelSize(R.styleable.ProgressBar_progressBarTextSize, 25);
        ...
        progressBarSaleTime = mTypedArray.getString(R.styleable.ProgressBar_progressBarSaleTime);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (progressBarIsOver) {
            float smallTextSize = DensityUtil.sp2px(getContext(), 2);
            paint.setColor(overTextColor);
            paint.setTextSize(textSize - smallTextSize);
            textHeight = textSize - smallTextSize;
            progressText = "已抢光";
        ...
        } else if (progressBarSaleTime != null && !"".equals(progressBarSaleTime)) {
            // 未开售
            float smallTextSize = DensityUtil.sp2px(getContext(), 6);
            paint.setColor(textColor);
            paint.setTextSize(textSize - smallTextSize);
            textHeight = textSize - smallTextSize;
            String date[] = progressBarSaleTime.split(" ");
            float  dayWidth;
            float  timeWidth;
            if (date.length == 2) {
                dayWidth = paint.measureText(date[0]);
                timeWidth = paint.measureText(date[1]);
                // 画出开售时间
                canvas.drawText(date[0], x - dayWidth / 2, y, paint);
                canvas.drawText(date[1], x - timeWidth / 2, y + textHeight, paint);
            } else if (date.length == 1) {
                timeWidth = paint.measureText(date[0]);
                float textWidth = paint.measureText("即将开售");
                canvas.drawText(date[0], x - timeWidth / 2, y, paint);
                canvas.drawText("即将开售", x - textWidth / 2, y + textHeight, paint);
            }
        } else {
            float smallTextSize = DensityUtil.sp2px(getContext(), 5);
            paint.setColor(textColor);
    ...
    public String getProgressBarSaleTime() {
        return progressBarSaleTime;
    }

    public void setProgressBarSaleTime(String progressBarSaleTime) {
        this.progressBarSaleTime = progressBarSaleTime;
    }
