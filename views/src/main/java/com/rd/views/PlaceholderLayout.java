...
public class PlaceholderLayout extends FrameLayout {
    ...
    public void setStatus(@Flavour int status) {
        ...
        switch (status) {
            case SUCCESS:
                ...
                loadingVisibilityChanged(View.GONE);
                break;
            ...
            case LOADING:
                ...
                errorPage.setVisibility(View.GONE);
                networkPage.setVisibility(View.GONE);
                loadingVisibilityChanged(View.VISIBLE);
                break;
     ...
     * 设置加载页面的可见性
     */
    private void loadingVisibilityChanged(int visibility) {
        if (defineLoadingPage != null) {
            defineLoadingPage.setVisibility(visibility);
        } else {
            loadingPage.setVisibility(visibility);
            if (loadingPage instanceof HexagonProgressView) {
                if (visibility == View.VISIBLE) {
                    ((HexagonProgressView) loadingPage).start();
                } else {
                    ((HexagonProgressView) loadingPage).stop();
                }
            }
        }
    }
