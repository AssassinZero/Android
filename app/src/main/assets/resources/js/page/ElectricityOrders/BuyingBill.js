...
define(function (require, exports, module) {
	...
	var getListDate = function () {
		...
		function insetDate(data) {
			var tpl = require('/resources/tpls/ElectricityOrders/BuyingBill.tpl'); //载入tpl模板
			var template = Handlebars.compile(tpl);
			var html = template(data);
			$("#BuyingBill").html(html);
			operationClick(data); //操作功能函数
			comfn.operationClick(data); //公共操作功能函数
		}
		$.ajax({
			url: "/modules/billorder/action/bizOnlineOrderAction/getPageList.htm",
			type: "get",
			dataType: "json",
			data: {
				currentPage: 1,
			...
			success: function (data) {
				...
				if (data.page.total > 0) {
					...
					$("#kkpager-BuyingBill").html('<p style="text-align:center; margin:20px 0;">暂无数据</p>');
				}
				// End-分页插件
	...
	function operationClick(checkData) {
		require.async(['/resources/js/module/layer/skin/default/layer.css', '/resources/js/module/layer/layer.min'], function () {
            ...
            /**
				* [票据查看]
				*/ 
			$('#BuyingBill .lookUp').click(function (e) {
			...
			/**
				* [取消订单] 
				*/
			$('#BuyingBill .BuyingBillCancel').click(function () {
				layer.confirm("是否确认取消订单？", {
					...
					$.ajax({
						...
						dataType: "json",
						data: {
							orderNo: $(".buyersMemberClose").attr("data-id"),
						},
						success: function (result) {
							if (result.code == 200) {
								layer.msg("关闭", {
